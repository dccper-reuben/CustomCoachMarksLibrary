...
public class CoachMark {
    ...
    public void showCoachMark(final View targetView, final CoachBackgroundView.CutoutShape targetCutoutShape) {
        ...
        *  - align image to right margin add margin to center, text to left of img*/
        ...
        @SuppressLint("InflateParams") final View view = LayoutInflater.from(context).inflate(R.layout.relative_layout_template, null);
        ...
        if (rootView != null) {
            ...
            if (targetView == null) {
                ...
                if (targetCutoutShape != CoachBackgroundView.CutoutShape.CIRCLE) {
                    targetX = valArr[0] - cutoutPadding;
                    targetY = valArr[1] - statusHeight - cutoutPadding;
                } else {
                    targetX = valArr[0] + (targetView.getWidth() > targetView.getHeight() ? ((targetView.getWidth() - targetView.getHeight()) / 2) : (-1 * ((targetView.getWidth() - targetView.getHeight()) / 2))) - cutoutPadding;
                    targetY = valArr[1] - statusHeight - cutoutPadding;
                }
                ...
                *==================*/
                ...
                if (targetCutoutShape != CoachBackgroundView.CutoutShape.CIRCLE) {
                    targetViewParams = new RelativeLayout.LayoutParams(targetView.getWidth() + (2 * cutoutPadding), targetView.getHeight() + (2 * cutoutPadding));
                } else {
                    targetViewParams = new RelativeLayout.LayoutParams(targetView.getHeight() + (2 * cutoutPadding), targetView.getHeight() + (2 * cutoutPadding));
                }
        ...
        try {
            ...
            if (errorHandlerDelegate != null) {
                errorHandlerDelegate.rerunCoachMark();
            }
            else{
    ...
    void setErrorHandlerDelegate(CoachMarkPopupErrorHandlerDelegate errorHandlerDelegate) {
